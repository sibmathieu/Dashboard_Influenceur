...
def commentaires(request):
    ...
    while page_token is not None:
        try:
            uploads_list = youtube.channels().list(
                part="contentDetails",
                id=channel_id
            ).execute()
    ...
    blob = bucket.blob("historique/comments.csv")

    with NamedTemporaryFile(mode='w', delete=False, newline='') as temp_file:
        fieldnames = ['video_id', 'video_title', 'comments']
        ...
        for comment_dict in video_comments:
            ...
            comment = comment_dict['comments']

            writer.writerow({'video_id': video_id, 'video_title': video_title, 'comments': comment})
